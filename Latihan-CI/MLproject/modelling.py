...
if __name__ == "__name__":
    ...
    X_train, X_test, y_train, y_test = train_test_split(data.drop("Credit_Score", axis=1), data['Credit_Score'],
                                                        random_state=42, test_size=0.2)
    input_example = X_train[0:5]
    n_estimators = int(sys.argv[1]) if len(sys.argc)>1 else 505
    max_depth = int(sys.argv[2]) if len(sys.argc)>2 else 37
